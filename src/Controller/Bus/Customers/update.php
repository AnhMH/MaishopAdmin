...
if (!empty($id)) {
    // Edit
    $param['id'] = $id;
    $data = Api::call(Configure::read('API.url_customers_detail'), $param);
    ...
    $pageTitle = __('LABEL_CUSTOMER_UPDATE');
} else {
    // Create new
...
// Create breadcrumb
$listPageUrl = h($this->BASE_URL . '/customers');
$this->Breadcrumb->setTitle($pageTitle)
    ->add(array(
        'link' => $listPageUrl,
        'name' => __('LABEL_CUSTOMER_LIST'),
    ))
    ->add(array(
        'name' => $pageTitle,
    ));
